...
def get_model(height, nclass,learning_rate):
    ...
    # 全连接输出
    y_pred = Dense(nclass, name='blstm2_out_union', activation='softmax')(m)
    # 确定模型
...
def get_model_lstm(height, nclass,learning_rate):
    input = Input(shape=(height, None, 1), name='the_input')
    m = Conv2D(64, kernel_size=(3, 3), activation='relu', padding='same', name='conv1')(input)
    m = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='pool1')(m)
    m = Conv2D(128, kernel_size=(3, 3), activation='relu', padding='same', name='conv2')(m)
    ...
    m = MaxPooling2D(pool_size=(2, 2), strides=(2, 1), padding='valid', name='pool4')(m)
    m = Conv2D(512, kernel_size=(2, 2), activation='relu', padding='valid', name='conv7')(m)
    # conv_base = Model(inputs=input,outputs=conv_net)
    # Permute层将输入的维度按照给定模式进行重排，例如，当需要将RNN和CNN网络连接时，可能会用到该层。
    m = Permute((2, 1, 3), name='permute')(m)
    m = TimeDistributed(Flatten(), name='timedistrib')(m)
    # cnn之后链接双向GRU，双向GRU会输出固定长度的序列，这是一个encode的过程，之后再连接一个双向GRU，对该序列进行解码
    # 该序列的输出为长度为256的序列
    # cnn之后连接双向GRU
    m = Bidirectional(LSTM(rnnunit, return_sequences=True), name='blstm_1')(m)
    # 全连接层-rnnunit为全连接层的输出维度
    m = Dense(rnnunit, name='blstm1_dense_1', activation='linear')(m)
    # 连接双向GRU
    m = Bidirectional(LSTM(rnnunit, return_sequences=True), name='blstm_2')(m)
    # 全连接输出
    y_pred = Dense(nclass, name='blstm2_dense_2', activation='softmax')(m)
    # 确定模型
    basemodel = Model(inputs=input, outputs=y_pred)
...
def get_model_lstm_with_channels(height, nclass,learning_rate):
